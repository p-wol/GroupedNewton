...
def epoch_durations(rows: list[dict]) -> list[float]:
    rows = sorted((r for r in rows if "time" in r and "epoch" in r), key=lambda r: r["epoch"])
...
def main(argv: list[str]) -> int:
    ...
    for m in sorted(root.glob("**/metrics.json")):
        ...
        runs.append(
            {
                "dir": job_dir,
                "overrides": read_overrides(job_dir),
                "durations": durations,
                "peak": max((r.get("memory_peak", 0) for r in read_metrics(m)), default=0),
            }
        )
    ...
    varying = sorted(
        k
        for k in keys
        if len({next((o for o in r["overrides"] if o.startswith(k + "=")), None) for r in runs}) > 1
    )
    ...
    for r in runs:
        label = (
            " ".join(o for o in r["overrides"] if any(o.startswith(k + "=") for k in varying))
            or r["dir"].name
        )
        d = r["durations"]
        if not d:
            ...
            continue
        rows.append((statistics.median(d), label, len(d), min(d), max(d), r["peak"] / 2**20))
    ...
    if len(rows) > 1:
        ...
        print(
            "Read the plateau, not the minimum: pick the smallest setting whose median "
