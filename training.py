...
class Trainer:
    ...
    def build_optimizer(self, model):
        ...
        if args.optimizer.name == "SGD":
            ...
            updater = optimizers.SGDUpdate(
                model.parameters(), momentum=args_hg.momentum, dampening=args_hg.momentum_damp
            )
    ...
    def train(self, ckpt_name="last_ckpt", log_name="metrics"):
        ...
        for self.epoch in range(self.args.optimizer.epochs):
            ...
            if self.args.logs_hg.use:
                if self.args.logs_hg.test_float:
                    ...
                    self.logger.log_artifact(
                        TorchModel(logs, ext=".pkl"), f"Hg_logs_ext.{self.epoch:05}"
                    )
            ...
            if self.args.optimizer.name == "NewtonSummary" and not self.args.optimizer.hg.nologs:
                ...
                self.logger.log_artifact(
                    TorchModel(logs_last, ext=".pkl"), f"Hg_logs_last.{self.epoch:05}"
                )
                self.logger.log_artifact(
                    TorchModel(logs_mean, ext=".pkl"), f"Hg_logs_mean.{self.epoch:05}"
                )
                self.logger.log_artifact(
                    TorchModel(logs_total, ext=".pkl"), f"Hg_logs_total.{self.epoch:05}"
                )
                self.logger.log_artifact(
                    TorchModel(self.logs_nlls, ext=".pkl"), f"nlls_logs_total.{self.epoch:05}"
                )

                self.optimizer.reset_logs()
            elif self.args.optimizer.name == "NewtonSummaryFB":
                self.logger.log_artifact(
                    TorchModel(self.optimizer.logs, ext=".pkl"), f"Hg_logs_hgfb.{self.epoch:05}"
                )
    ...
    def compute_logs(self):
        ...
        direction = fullbatch_gradient(
            self.model, self.loss_fn, self.tup_params, self.hg_loader, self.train_size
        )
        ...
        if not self.dct_nesterov["use"]:
            ...
            lrs, r_root, r_converged = nesterov_lrs(
                H, g, order3, damping_int=self.dct_nesterov["damping_int"]
            )
            logs["nesterov.r"] = torch.tensor(r_root, device=self.device, dtype=self.dtype)
            logs["nesterov.converged"] = torch.tensor(
                r_converged, device=self.device, dtype=self.dtype
            )
