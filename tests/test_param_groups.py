...
def _build_tensors_from_shapes(lst_shapes):
    return [
        {"params": [torch.randn(*shape) for shape in group_shapes]} for group_shapes in lst_shapes
    ]
...
def test_dot_partial(lst_shapes, start, end):
    ...
    res_custom = param_groups1.dot(
        param_groups1.select_params(start=start, end=end),
        param_groups2.select_params(start=start, end=end),
        start=start,
        end=end,
    )
