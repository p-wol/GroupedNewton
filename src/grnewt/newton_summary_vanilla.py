    ...
    def step(self):
        ...
        if self.step_counter % self.period_hg == 0:
            ...
            if not self.autoencoder:
                y_target = y_target.to(
                    device=self.device
                )  # XXX: dtype: this conversion has to be explicit for the user
            else:
                y_target = x
            ...
            if self.noregul or not self.dct_nesterov["use"]:
                ...
                lrs, lrs_logs = nesterov_lrs(
                    H, g, order3_, damping_int=self.dct_nesterov["damping_int"], clip_r=clip_r
                )
            ...
            if not perform_update:
                ...
                if self.dct_lrs_clip["mode"] == "movavg":
                    ...
                    NotImplementedError(
                        "Error: unknown mode for lrs_clip: {}".format(self.dct_lrs_clip["mode"])
                    )
