...
def nesterov_lrs(
    H, g, order3_, *, damping_int=1.0, force_x0_computation=None, threshold_D_sing=1e-5
):
    """
    ...
    def f(x):
        return (D @ torch.linalg.solve(H + 0.5 * damping_int * x * D_squ, g)).norm().item() - x

    # Compute x0
    x0, dct_logs_x0 = compute_x0(
        H,
        order3_,
        D_squ,
        damping_int,
        threshold_D_sing=threshold_D_sing,
        force_x0_computation=force_x0_computation,
    )
    ...
    # Compute 'lrs' from 'r'
    lrs = torch.linalg.solve(H + 0.5 * damping_int * r_root * D_squ, g)
    dct_logs["lrs"] = lrs
...
def compute_x0(H, order3_, D_squ, damping_int, threshold_D_sing=1e-5, threshold_H_sing=1e-10,
    ...
    if not H_pd and damping_int == 0:
        raise ValueError(
            "H is not positive definite and damping_int == 0: case impossible to solve. You may try damping_int > 0."
        )
    ...
    if force_x0_computation is None:
        ...
        if force_x0_computation in ["Direct_Hpd", "Analytical", "Numerical"]:
            ...
            raise ValueError(
                f'Error: unknown value for "force_x0_computation", found {force_x0_computation}.'
            )
    ...
    if x0_computation == "Direct_Hpd":
        ...
        # TODO: explain
        def fn_g(x):
            return torch.linalg.eigvalsh(H + 0.5 * damping_int * x * D_squ).min().item()
