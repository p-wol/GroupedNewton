...
def create_loaders(args, dct):
    ...
    dct["trainset"], dct["validset"] = tuple(
        data.random_split(dct["tvset"], [dct["train_size"], dct["valid_size"]])
    )

    dsloader = args.dsloader

    # Create loaders
    dct["train_loader"] = data.DataLoader(
        dct["trainset"],
        args.dataset.batch_size,
        shuffle=True,
        num_workers=dsloader.num_workers,
        persistent_workers=dsloader.num_workers > 0,
        pin_memory=dsloader.pin_memory,
    )
    dct["valid_loader"] = data.DataLoader(
        dct["validset"],
        args.dataset.batch_size,
        num_workers=dsloader.num_workers_eval,
        pin_memory=dsloader.pin_memory,
    )
    dct["test_loader"] = data.DataLoader(
        dct["testset"],
        args.dataset.batch_size,
        num_workers=dsloader.num_workers_eval,
        pin_memory=dsloader.pin_memory,
...
def build_MNIST(args, dct):
    ...
    dct["tvset"] = torchvision.datasets.MNIST(
        root=args.dataset.path, train=True, download=False, transform=transform_train
    )

    dct["testset"] = torchvision.datasets.MNIST(
        root=args.dataset.path, train=False, download=False, transform=transform_test
    )
...
def build_CIFAR10(args, dct):
    ...
    dct["tvset"] = torchvision.datasets.CIFAR10(
        root=args.dataset.path, train=True, download=False, transform=transform_train
    )

    dct["testset"] = torchvision.datasets.CIFAR10(
        root=args.dataset.path, train=False, download=False, transform=transform_test
    )
...
def build_ImageNet(args, dct):
    ...
    dct["tvset"] = torchvision.datasets.ImageNet(
        root=args.dataset.path, split="train", transform=transform_train
    )

    dct["testset"] = torchvision.datasets.ImageNet(
        root=args.dataset.path, split="val", transform=transform_test
    )
...
def build_toy_regression(args, dct):
    if args.model.name != "Perceptron":
        raise ValueError(
            f'Error: with dataset "ToyRegression", the model must be "Perceptron", got {args.model.name}.'
        )
