...
class NewtonSummaryFB(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        lrs = self.compute_lrs(
            direction, nesterov_damping=self.dct_nesterov["damping_int"], noregul=self.noregul
        )
    ...
    def compute_lrs(self, direction, *, nesterov_damping=None, noregul=False):
        ...
        if noregul:
            ...
            lrs, r_root, r_converged = nesterov_lrs(H, g, order3, damping_int=nesterov_damping)
            ...
            self.logs["nesterov.converged"] = torch.tensor(
